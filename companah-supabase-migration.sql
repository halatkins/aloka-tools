-- ============================================================
-- COMPANAH — Supabase Schema Migration (v4)
-- Project: Aloka (qrnjwdbqzeqtcyzwljvu)
--
-- Run this in the Supabase SQL Editor:
--   Dashboard → SQL Editor → New Query → Paste → Run
--
-- This script creates all schemas, tables, indexes, and
-- triggers for the Companah database. Safe to run on a
-- fresh project — uses IF NOT EXISTS throughout.
-- ============================================================
...
ALTER DEFAULT PRIVILEGES IN SCHEMA pricing GRANT SELECT ON TABLES TO anon, authenticated;
...
ALTER DEFAULT PRIVILEGES IN SCHEMA pricing GRANT ALL ON TABLES TO service_role;
...
-- Add schemas to the PostgREST search path
...
CREATE TABLE IF NOT EXISTS core.pets (
  ...
  name                text,
  species             text CHECK (species IN ('dog', 'cat', 'bird', 'reptile', 'other')),
  breed               text,
  color               text,
...
COMMENT ON TABLE core.pets IS 'The animals — linked to their owner contact';


-- ════════════════════════════════════════════════════════════
-- STEP 3: PRICING SCHEMA — Service catalog & rates
-- ════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS pricing.products (
  ...
  description         text,
  sku                 text,
  category            text CHECK (category IN ('cremation', 'addon', 'urn', 'keepsake', 'paw_print', 'delivery')),
  product_type        text CHECK (product_type IN ('service', 'non_inventory')),
  default_price       numeric,
  is_taxable          boolean DEFAULT false,
  qbo_income_account  text,
  qbo_expense_account text,
  is_active           boolean DEFAULT true,
  created_at          timestamptz NOT NULL DEFAULT now(),
  updated_at          timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX idx_products_sku ON pricing.products(sku);
CREATE INDEX idx_products_category ON pricing.products(category);
CREATE INDEX idx_products_active ON pricing.products(is_active);

CREATE TRIGGER products_updated_at
  BEFORE UPDATE ON pricing.products
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();

COMMENT ON TABLE pricing.products IS 'Product/service catalog with SKUs and QBO account mapping';

-- ── pricing.partner_rates ───────────────────────────────────
CREATE TABLE IF NOT EXISTS pricing.partner_rates (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL REFERENCES core.organizations(id) ON DELETE CASCADE,
  service_type    text CHECK (service_type IN ('individual', 'communal')),
  weight_range    text CHECK (weight_range IN ('0-25', '26-50', '51-100', '100+')),
...
CREATE INDEX idx_partner_rates_org ON pricing.partner_rates(organization_id);

COMMENT ON TABLE pricing.partner_rates IS 'Per-partner wholesale rates by service type and weight range';


-- ════════════════════════════════════════════════════════════
-- STEP 4: ORDERS SCHEMA — Cremation order lifecycle
-- ════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS orders.orders (
  ...
  client_contact_id     uuid REFERENCES core.contacts(id) ON DELETE SET NULL,
  vet_organization_id   uuid REFERENCES core.organizations(id) ON DELETE SET NULL,
...
CREATE INDEX idx_orders_client ON orders.orders(client_contact_id);
CREATE INDEX idx_orders_vet_org ON orders.orders(vet_organization_id);
...
COMMENT ON TABLE orders.orders IS 'Cremation orders from intake to delivery';

-- ── orders.line_items ───────────────────────────────────────
CREATE TABLE IF NOT EXISTS orders.line_items (
  id          uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  order_id    uuid NOT NULL REFERENCES orders.orders(id) ON DELETE CASCADE,
  product_id  uuid REFERENCES pricing.products(id) ON DELETE SET NULL,
  description text,
...
COMMENT ON TABLE orders.line_items IS 'Products/services included in an order with prices at time of order';
...
COMMENT ON TABLE orders.care_plans IS 'Care plan details — vessel, prints, engraving, memorial package';
...
-- ════════════════════════════════════════════════════════════
-- STEP 5: OPERATIONS SCHEMA — Cremation processing
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- STEP 6: BILLING SCHEMA — QBO integration
-- ════════════════════════════════════════════════════════════
...
CREATE INDEX idx_invoices_client ON billing.invoices(client_contact_id);
CREATE INDEX idx_invoices_vet_org ON billing.invoices(vet_organization_id);
...
COMMENT ON TABLE billing.invoices IS 'Invoice headers with QBO sync fields';
...
COMMENT ON TABLE billing.invoice_line_items IS 'Line items on an invoice — product, qty, rate, tax';


-- ════════════════════════════════════════════════════════════
-- STEP 7: WORKFLOWS SCHEMA — Task management
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- STEP 8: PARTNERS SCHEMA — Vet partner portal
-- ════════════════════════════════════════════════════════════
...
COMMENT ON TABLE partners.profiles IS 'Partner-specific config — tier, billing, wholesale rates, portal';
...
-- ════════════════════════════════════════════════════════════
-- STEP 9: CLIENTS SCHEMA — Client portal & reviews
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- STEP 10: LOGISTICS SCHEMA — Delivery & pickup tracking
-- ════════════════════════════════════════════════════════════
...
--
-- 9 schemas:
--   core, pricing, orders, operations, billing,
--   workflows, partners, clients, logistics
--
-- 22 tables:
--   core:       organizations, contacts, pets
--   pricing:    products, partner_rates
...
--
-- Next steps:
--   1. Enable RLS on each table via the Supabase dashboard
--   2. Expose schemas in API settings (Settings → API → Schema)
--   3. Run the Airtable migration to populate data
-- ════════════════════════════════════════════════════════════
